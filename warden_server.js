...
app.get("/", (req, res) => {
    if (req.session.user) {
      res.redirect("warden_login.html");
    } else {
      res.sendFile(path.join(__dirname, "public", "warden_login.html"));
    }
  });
...
app.post("/Login", async (req, res) => {
    ...
    try {
      const result = await db.query(
        "SELECT * FROM wardens WHERE Upper(regno) = $1",
        [email]
      );
